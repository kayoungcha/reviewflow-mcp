  ...
  async () => {
    try {
      ...
      return {
        isError: true,
        content: [
          { type: "text", text: `최근 커밋을 조회하지 못했습니다: ${message}` },
        ],
      };
    }
  },
...
server.registerTool(
  ...
  {
    description:
      "현재 Git 저장소에서 아직 커밋하지 않은 변경사항의 diff를 조회합니다. 사용자가 현재 변경사항, 수정 내용 또는 diff 요약을 요청할 때 사용합니다.",
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
    },
  ...
  async () => {
    try {
      ...
      return {
        content: [
          { type: "text", text: diff || "커밋되지 않은 변경사항이 없습니다." },
        ],
      };
    } catch (error: unknown) {
      const message =
      ...
      return {
        isError: true,
        content: [
          { type: "text", text: `Git diff를 조회하지 못했습니다: ${message}` },
        ],
      };
    }
  },
...
server.registerTool(
  ...
  async () => {
    try {
      ...
      return {
        content: [{ type: "text", text: status || "변경된 파일이 없습니다." }],
      };
    } catch (error: unknown) {
      const message =
        error instanceof Error ? error.message : "알 수 없는 오류";
